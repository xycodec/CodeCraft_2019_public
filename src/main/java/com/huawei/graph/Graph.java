...
import com.huawei.utils.CrossInfo;
import com.huawei.utils.Direction;
import com.huawei.utils.Path;
import com.huawei.utils.LinkPair;
import com.huawei.utils.NodePair;
import com.huawei.vehicle.Car;
import com.huawei.vehicle.CarComparator;

public class Graph {
	public static double INF=100000000;
	public ArrayList<CrossNode> head=new ArrayList<>();//节点集合
	Map<NodePair, RoadLink> node_pair_to_link=new HashMap<>();//节点对映射到道路(两个node决定一个link)
	Map<LinkPair,Integer> link_pair_to_node=new HashMap<>();//映射连接link_1,link_2的cross_id
	Map<NodePair,Double> node_pair_to_link_tmp=new HashMap<>();//节点对映射到cost
	Map<LinkPair,Direction> dir_mp=new HashMap<>();//方向映射,不写调度器的话暂时用不到
	public Graph() {
	...
	Map<Integer, NodePair> id_to_node_pair=new HashMap<>();//记录单向边road_id与对应的节点对的映射
	public void init3(String carPath,String roadPath,
			String crossPath,String preAnsPath) {
		CrossInfo.read(carPath,roadPath,crossPath,preAnsPath);
		
		ArrayList<ArrayList<Integer>> road_info= CrossInfo.Roads;
		ArrayList<ArrayList<Integer>> cross_info= CrossInfo.Crosses;
		for(int i=0;i<cross_info.size()+1;++i) {//添加节点
			head.add(new CrossNode(i));
		}
		...
		for(int i=0;i<road_info.size();++i) {
			...
			if(is_double==1) {//双向边src_id->dst_id,dst_id->src_id
				RoadLink tmp1=new RoadLink(road_id, length, speed_limit, lane_num, src_id, dst_id);
				RoadLink tmp2=new RoadLink(road_id, length, speed_limit, lane_num, dst_id, src_id);
				//src->dst
				//real_id_mp.get(src_id),src_id本身可能不连续或者是其他乱七八糟的数据,把它映射到连续的数字上,数字从1开始....,下同
				CrossNode tmp_node=new CrossNode(dst_id);//这里不用映射,dst_id是cross_node的本身属性,下同
				...
				NodePair tmp_node_sd=new NodePair(src_id, dst_id);
				NodePair tmp_node_ds=new NodePair(dst_id, src_id);
				...
				id_to_node_pair.put(road_id, new NodePair(src_id, dst_id));
				
				RoadLink tmp1=new RoadLink(road_id, length, speed_limit, lane_num, src_id, dst_id);
				//src->dst
				CrossNode tmp_node=new CrossNode(dst_id);
				if(head.get(real_id_mp.get(src_id)).next_node==null) {
					head.get(real_id_mp.get(src_id)).next_node=tmp_node;
				}else {
					tmp_node.next_node=head.get(real_id_mp.get(src_id)).next_node;
		...
		for(int i=0;i<cross_info.size();++i) {
			...
			if(up_id!=-1) {
				if(id_to_double.containsKey(up_id)) {//是双向边
					...
					NodePair tmp_node=id_to_node_pair.get(up_id);//根据单向边的road_id得到node_pair
				...
				if(right_id!=-1) {
					dir_mp.put(new LinkPair(up_id, right_id), Direction.LEFT);
					link_pair_to_node.put(new LinkPair(up_id, right_id), cross_id);
				}
				if(down_id!=-1) {
					dir_mp.put(new LinkPair(up_id, down_id), Direction.DIRECT);
					link_pair_to_node.put(new LinkPair(up_id, down_id), cross_id);
				}
				if(left_id!=-1) {
					dir_mp.put(new LinkPair(up_id, left_id), Direction.RIGHT);
					link_pair_to_node.put(new LinkPair(up_id, left_id), cross_id);
				}
			}
			if(right_id!=-1) {
				if(id_to_double.containsKey(right_id)) {//是双向边
					...
					NodePair tmp_node=id_to_node_pair.get(right_id);//根据单向边的road_id得到node_pair
					if(tmp_node.src_id==cross_id) {//出边
						//出度
						if(!id_to_degree_out.containsKey(cross_id))
							id_to_degree_out.put(cross_id, 1);
				...
				if(up_id!=-1) {
					dir_mp.put(new LinkPair(right_id, up_id), Direction.RIGHT);
					link_pair_to_node.put(new LinkPair(right_id, up_id), cross_id);
				}
				if(down_id!=-1) {
					dir_mp.put(new LinkPair(right_id, down_id), Direction.LEFT);
					link_pair_to_node.put(new LinkPair(right_id, down_id), cross_id);
				}
				if(left_id!=-1) {
					dir_mp.put(new LinkPair(right_id, left_id), Direction.DIRECT);
					link_pair_to_node.put(new LinkPair(right_id, left_id), cross_id);
				}
			}
			if(down_id!=-1) {
				if(id_to_double.containsKey(down_id)) {//是双向边
					...
					NodePair tmp_node=id_to_node_pair.get(down_id);//根据单向边的road_id得到node_pair
					if(tmp_node.src_id==cross_id) {//出边
						//出度
						if(!id_to_degree_out.containsKey(cross_id))
							id_to_degree_out.put(cross_id, 1);
				...
				if(up_id!=-1) {
					dir_mp.put(new LinkPair(down_id, up_id), Direction.DIRECT);
					link_pair_to_node.put(new LinkPair(down_id, up_id), cross_id);
				}
				if(right_id!=-1) {
					dir_mp.put(new LinkPair(down_id, right_id), Direction.RIGHT);
					link_pair_to_node.put(new LinkPair(down_id, right_id), cross_id);
				}
				if(left_id!=-1) {
					dir_mp.put(new LinkPair(down_id, left_id), Direction.LEFT);
					link_pair_to_node.put(new LinkPair(down_id, left_id), cross_id);
				}
			}
			if(left_id!=-1) {
				if(id_to_double.containsKey(left_id)) {//是双向边
					...
					NodePair tmp_node=id_to_node_pair.get(left_id);//根据单向边的road_id得到node_pair
					if(tmp_node.src_id==cross_id) {//出边
						//出度
						if(!id_to_degree_out.containsKey(cross_id))
							id_to_degree_out.put(cross_id, 1);
				...
				if(up_id!=-1) {
					dir_mp.put(new LinkPair(left_id, up_id), Direction.LEFT);
					link_pair_to_node.put(new LinkPair(left_id, up_id), cross_id);
				}
				if(right_id!=-1) {
					dir_mp.put(new LinkPair(left_id, right_id), Direction.DIRECT);
					link_pair_to_node.put(new LinkPair(left_id, right_id), cross_id);
				}
				if(down_id!=-1) {
					dir_mp.put(new LinkPair(left_id, down_id), Direction.RIGHT);
					link_pair_to_node.put(new LinkPair(left_id, down_id), cross_id);
				}
			}
		}
...
// 		}
		...
		double max_degree=-1;
		for(NodePair tmp:node_pair_to_link.keySet()) {
		...
		//degree越大,node越稳健
		for(RoadLink tmp:node_pair_to_link.values()) {
//			System.out.printf("road_%d\tdegree:%f\n",tmp.road_id,tmp.degree);
	...
	public Path Dijkstra(int v0,int t){//v0-> otherNode
		...
		CrossNode tmp_node=head.get(v0).next_node;
		while(tmp_node!=null) {//初始化
			dist[real_id_mp.get(tmp_node.cross_id)]=node_pair_to_link.get(new NodePair(r_real_id_mp.get(v0), tmp_node.cross_id)).cost;
		...
		for(int i=1;i<head.size()-1;++i) {
			...
			while(tmp_node!=null) {
				RoadLink tmp_link=node_pair_to_link.get(new NodePair(r_real_id_mp.get(u), tmp_node.cross_id));
...
//		System.out.println(v0+"->"+t);
	...
	public void set_link(ArrayList<Integer> p1,int s_node) {//设置偏离路径为INF,p1:待检测的path,s_node:p1的末结点
		...
		for(int i=0;i<len;++i) {
			...
			//前面的结点都一样,然后就看扩展结点的了
			CrossNode tmp_node=head.get(real_id_mp.get(s_node)).next_node;
			while(tmp_node!=null) {//从s_node扩展结点
				if(tmp_node.cross_id==p2.get(len_p)) {
					node_pair_to_link_tmp.put(new NodePair(s_node,p2.get(len_p)), INF);
				}
				tmp_node=tmp_node.next_node;
			}
		}
	...
	public void recover_link(int s_node) {
		CrossNode tmp_node=head.get(real_id_mp.get(s_node)).next_node;
		while(tmp_node!=null) {
			node_pair_to_link_tmp.put(new NodePair(s_node, tmp_node.cross_id), node_pair_to_link.get(new NodePair(s_node, tmp_node.cross_id)).cost);
			tmp_node=tmp_node.next_node;
		}
		for(int i=1;i<head.size();++i) {
			s[i]=false;
	...
	public void Yen_ksp(int s,int t,int k) {
		...
		//copy
		for(int i=1;i<head.size();++i){
			CrossNode tmp_node=head.get(i).next_node;
			while(tmp_node!=null) {
				node_pair_to_link_tmp.put(new NodePair(r_real_id_mp.get(i), tmp_node.cross_id), node_pair_to_link.get(new NodePair(r_real_id_mp.get(i), tmp_node.cross_id)).cost);
				tmp_node=tmp_node.next_node;
			}
		}
		...
		while(k-->0) {
			...
			for(int i=0;i<len-1;++i) {
				if(i>=1) {
					dist_tmp+=node_pair_to_link_tmp.get(new NodePair(p.path.get(i-1),p.path.get(i)));
				}
	...
	public Map<NodePair,Integer> ni_mp=new HashMap<>();//想办法统计出(s,t)的频率,即从起点s到终点t的车子数目,尽量让它们分散开
	public Map<NodePair,ArrayList<Car>> nc_mp=new HashMap<>();//节点对(起点到终点)映射到车子(起点到终点是该节点对)的集合
	...
	public static void calc4(String carPath,String roadPath,
		...
		ArrayList<ArrayList<Integer>> pre_ans= CrossInfo.PreAns;//预置车辆
		...
		Random r=new Random();
		ArrayList<ArrayList<Integer>> car_info= CrossInfo.Cars;//车子信息
		...
		for(int i=0;i<car_info.size();++i) {
			...
			NodePair tmp_node=new NodePair(tmp_car.from, tmp_car.to);
			...
			if(g1.ni_mp.containsKey(tmp_node))
				g1.ni_mp.put(tmp_node, 1+g1.ni_mp.get(new NodePair(tmp_car.from, tmp_car.to)));
			else
		...
		for(NodePair tmp:g1.ni_mp.keySet()) {
//			System.out.printf("(%d,%d) : %d\n",tmp.src_id,tmp.dst_id,g1.ni_mp.get(tmp));
			if(g1.ni_mp.get(tmp)>50) {
				for(Car c:g1.nc_mp.get(new NodePair(tmp.src_id, tmp.dst_id))) {
					c.is_hard=true;
					++cnt;
				}
			}
...
//		for(int i=0;i<5;++i) Collections.shuffle(cars);
		...
		Collections.sort(preset_cars, new CarComparator());
...
//				System.out.println(tmp.get(j)+" "+tmp.get(j+1)+":"+g1.link_pair_to_node.get(new link_pair(tmp.get(j), tmp.get(j+1))));
				...
				tmp2.add(g1.link_pair_to_node.get(new LinkPair(tmp.get(j), tmp.get(j+1))));
			}
			tmp2.add(car_to);//完整的node_id序列应包含起点与终点
			for(int j=0;j<tmp2.size()-1;++j) {
				NodePair tmp_node=new NodePair(tmp2.get(j), tmp2.get(j+1));
//				System.out.println(tmp2.get(j)+" "+tmp2.get(j+1));
		...
		for(int i=0;i<(s_time+30)*16;++i) {
			...
			g1.Yen_ksp(s, t, Constant.candidate_paths);//计算若干条备选路
//			System.out.println(s+"->"+t);
			...
			for(int j=0;j<p.path.size()-1;++j) {
				NodePair tmp_node=new NodePair(p.path.get(j),p.path.get(j+1));
				RoadLink tmp_link=g1.node_pair_to_link.get(tmp_node);
				++tmp_link.used_cnt;
...
//			}
			for(NodePair tmp:g1.node_pair_to_link.keySet()) {
			...
			for(int j=0;j<p.path.size()-2;++j) {
				pout.print(g1.node_pair_to_link.get(new NodePair(p.path.get(j),p.path.get(j+1))).road_id+",");
			}
			pout.println(g1.node_pair_to_link.get(new NodePair(p.path.get(p.path.size()-2),p.path.get(p.path.size()-1))).road_id+")");

			//根据每条road_link的used_cnt进行反馈,调整链路的cost
			if(priority_cars.get(i).is_hard) {
				for(int j=0;j<p.path.size()-1;++j) {
					g1.node_pair_to_link.get(new NodePair(p.path.get(j),p.path.get(j+1))).update_cost(cars.size()/1.2);
				}
			}else {
				for(int j=0;j<p.path.size()-1;++j) {
					g1.node_pair_to_link.get(new NodePair(p.path.get(j),p.path.get(j+1))).update_cost(cars.size());
				}
			}
			g1.clear();
		}
